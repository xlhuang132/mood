...
def ood_inject(ul_train,ood_root,ood_r=0.0,ood_dataset='TIN',include_all=False):
    ood_dataset=ood_dataset_map[ood_dataset]  
    OOD = np.load(os.path.join(ood_root,ood_dataset+'.npy'))
    total_num=len(ul_train["images"])
    if include_all:
        images=list(ul_train["images"])
        labels=list(ul_train["labels"])
        images.extend(OOD)
        labels=labels+[-1]*len(OOD) 
